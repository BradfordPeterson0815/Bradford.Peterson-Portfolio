...
test.describe(
  'Claim Media Page',
  {
    tag: [Tags.UserPortal, Tags.Claim, Tags.Media],
  },
  () => {
    test('Verify Navigation and UI Elements', async ({ browser }) => {
      ...
      await mediaPage.VerifyClaimNumber()
      await mediaPage.Media.VerifyTitle()
      await expect(mediaPage.Media.Link_UploadMedia.locator).toBeVisible()
      await mediaPage.page.waitForTimeout(3000)

      if (!(await mediaPage.Media.IsMediaPageEmpty())) {
        // check the card count - should be at least as many as the canned claim data
        const mediaCardCount = await mediaPage.Media.MediaCardCount()
        expect(mediaCardCount).toBeGreaterThanOrEqual(claim.testData.mediaCards.length)
        ...
        await mediaPage.Media.VerifyAndFetchMediaCard(claim.testData.mediaCards[kittyV1CannedIndex])

        // check the Kitty v2 card
        const kittyV2CannedIndex = 1
        // will throw an error if we don't find the card with all the data matching
        await mediaPage.Media.VerifyAndFetchMediaCard(claim.testData.mediaCards[kittyV2CannedIndex])
        ...
        await mediaPage.Media.VerifyAndFetchMediaCard(
          claim.testData.mediaCards[piggyCannedIndex],
          true
        )
      } else {
        // if the page is empty, check the expected UI
        await expect(mediaPage.Media.Label_Empty_Title.locator).toBeVisible()
        await mediaPage.Media.Label_Empty_Title.VerifyExpectedText()
        await mediaPage.Media.Label_Empty_Description.VerifyExpectedText()
        await expect(mediaPage.Media.Link_Empty_UploadMedia.locator).toBeVisible()
      }
    })
    ...
    test('Media Card - Verify Media Preview Link', async ({ browser }) => {
      ...
      if (await mediaPage.Media.IsMediaPageEmpty()) {
        AbortTest(AbortErrors.EmptyMediaPageMessage)
        return
      }

      // grab the Kitty v1 card
      const kittyV1CannedIndex = 0
      const { card: kittyCard } = await mediaPage.Media.VerifyAndFetchMediaCard(
        claim.testData.mediaCards[kittyV1CannedIndex]
      )
    ...
    test('Media Card - Update Document Information: Verify Drawer UI', async ({ browser }) => {
      ...
      const piggyCannedIndex = 2
      const { card: piggyCard } = await mediaPage.Media.FetchMediaCardByFilename(
        claim.testData.mediaCards[piggyCannedIndex].filename
      )
    ...
    test('Media Card - Update Document Information', async ({ browser }) => {
      ...
      const piggyCannedIndex = 2
      const { card: piggyInitialCard } = await mediaPage.Media.FetchMediaCardByFilename(
        claim.testData.mediaCards[piggyCannedIndex].filename
      )
      const dateSuffix = `+${Date.now()}`
      const modifiedDescription = `${claim.testData.mediaCards[piggyCannedIndex].description}${dateSuffix}`
      ...
      const { card: piggyUpdatedCard } = await mediaPage.Media.FetchMediaCardByFilename(
        claim.testData.mediaCards[piggyCannedIndex].filename
      )
