...
import { DelegatePortalDeleteAlert } from './alerts/delegatePortalDeleteAlert.js'
import { AlertStrings } from './delegatePortalConstants.js'
...
export class DelegatePortalPhotoReportGroup extends DelegatePortalBase {
  ...
  constructor(global: DelegatePortalGlobal, parent: Locator, groupIndex: number) {
    super(global)
  ...
  async FetchCardByIndex(index: number) {
    const card = new DelegatePortalPhotoReportCard(this.global, this.cardParent, index)
    return card
  }
  ...
  async HandleDeleteGroupAlert(cancelDelete = false) {
    const alert = new DelegatePortalDeleteAlert(
      this.global,
