...
export class ClaimsPortalJobDataTable extends ClaimsPortalSelectionDataTable {
  ...
  async IsColumnPinnable(columnType: DataTable_Columns_Type) {
    return await this.IsColumnPinnableByName(LookupDataColumn(columnType))
  }
  ...
  async SetColumnPinState(columnType: DataTable_Columns_Type, pinState: DataTable_Column_PinState) {
    await this.SetColumnPinStateByName(LookupDataColumn(columnType), pinState)
  }

  async SetColumnPinStateByName(columnName: string, pinState: DataTable_Column_PinState) {
    const currentPinState = await this.FetchColumnPinStateByName(columnName)
    if (
  ...
  async FetchColumnPinState(columnType: DataTable_Columns_Type) {
    return this.FetchColumnPinStateByName(LookupDataColumn(columnType))
  }

  async FetchColumnPinStateByName(columnName: string) {
    const buttonLocator = this.table.locator(
      `th[id*='_HeaderRow_0_HeaderCell_${columnName}'] button[aria-label="Toggle pin column."]`
    )
  ...
  async OpenAssignContact(contactType: JobAssignContactOptions): Promise<ClaimsPortalAssignContactDialog> {
  ...
  async TagIsAddedByIndex(rowIndex: string, tag: string, value: string = '') {
    return this.TagWithValueIsAddedByIndex(rowIndex, tag, value)
  }
