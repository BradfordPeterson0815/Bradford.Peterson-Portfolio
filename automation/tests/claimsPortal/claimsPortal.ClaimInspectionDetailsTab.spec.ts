...
test.describe(
  ...
  {
    tag: [Tags.ClaimsPortal, Tags.Claim, Tags.Inspection, Tags.InfoDetails],
  },
  () => {
    ...
    test('Inspection Screenshots Table - Settings: Verify UI', async ({ browser }) => {
      ...
      const testClaim = FetchCannedClaim(environment, CannedClaimTypes.DefaultTestClaim)
      const claimPage = new ClaimsPortalClaimPage(global, testClaim)
      const inspectionDetailsTab = new ClaimsPortalInspectionDetailsTab(global, claimPage.baseURL)
    ...
    test('Inspection Screenshots Table - Table Filter: Verify UI', async ({ browser }) => {
      ...
      //If the table is empty, we cannot perform this test
      if (await table.IsEmpty()) {
        AbortTest(AbortErrors.EmptyClaimInspectionDetailsScreenshotsTableMessage)
        return
      }
    ...
    test('Inspection Screenshots Table - Table Filter: Add Filter', async ({ browser }) => {
      ...
      const labelFilterTerm = 'No Match Expected'
      ...
      // Verify table is filtered
      const labelFilteredRowCount = await table.VisibleRowCount()
      expect(labelFilteredRowCount).toBeLessThanOrEqual(initialRowCount)

      // Clear the filter selection
      await tableFilterDialog.Button_ClearFilter.Click()

      // Verify column is NOT filtered
      const labelFilterOffRowCount = await table.VisibleRowCount()
      expect(labelFilterOffRowCount).toBe(initialRowCount)
    ...
    test('Inspection Screenshots Table - Edit label and description inline', async ({
      ...
      const rowIndex = '0'
      const leftElevation = 'Left Elevation'.toUpperCase()
      const rightElevation = 'Right Elevation'.toUpperCase()
      ...
      const cannedLabelToUse = labelBeforeEdit == leftElevation ? rightElevation : leftElevation
      ...
      await table.VerifyRowTextDataByColumnName(
        cannedLabelToUse,
        rowIndex,
        DataTable_Columns_Type.InspectionScreenshots_Label
      )
    ...
    test('Transcript Search', async ({ browser }) => {
      ...
      await inspectionDetailsTab.PerformSearch('fuzzy slippers')
