...
import { ClaimsPortalJobDocumentsTab } from '../../library/claimsPortal/tabs/claimsPortalJobDocumentsTab.js'
...
test.describe(
  ...
  {
    tag: [Tags.ClaimsPortal, Tags.Job, Tags.Documents],
  },
  () => {
    test('Smoke Test', { tag: [Tags.Smoke, Tags.P1] }, async ({ browser }) => {
      ...
      // Verify that we land on the Job page for that job (Info tab by default), then navigate to the Documents tab
      const documentsTab = (await jobPage.SelectJobTab(JobTabTypes.Documents)) as ClaimsPortalJobDocumentsTab
    ...
    test('Documents Table - Settings: Verify UI', async ({ browser }) => {
      ...
      //Verify that we land on the Job page for that job, then navigate to the Documents tab
      const documentsTab = (await jobPage.SelectJobTab(JobTabTypes.Documents)) as ClaimsPortalJobDocumentsTab
      const table = documentsTab.DataTable_Documents
    ...
    test.describe('serial tests', () => {
      ...
      test('Documents Table - Settings: Verify Columns', async ({ browser }) => {
        ...
        )) as ClaimsPortalJobDocumentsTab
        const table = documentsTab.DataTable_Documents
    ...
    test('Documents Table - Global Search: Verify search', async ({ browser }) => {
      ...
      // Verify setting search input causes the table results to filter across all text fields
      const fileSearchTerm = testJob.testData.claimsDocument
      ...
      const descriptionSearchTerm = 'No match expected'
    ...
    test('Documents Table - Table Filter: Add Filter', async ({ browser }) => {
      ...
      //If the table is empty, we cannot perform this test
      if (await table.IsEmpty()) {
        AbortTest(AbortErrors.EmptyJobDocumentsTableMessage)
        return
      }
      ...
      const fileFilterTerm = testJob.testData.claimsDocument
    ...
    test('Documents Table  - Verify Action Menu Visibility: Update/Delete Document', async ({
      browser,
    }) => {
      // launch the Claims Portal
      const { global } = await Launch(browser, environment)
      ...
      // Filter for a non-ClaimsPortal Data Source document
      const { pinnedFilter: pinnedRedactedFilter } = await documentsTab.SetTableFilter_Selection(
        Documents_Meta_DataSourceSelectionOptions.UserPortal,
        DataTable_Columns_Type.Documents_Meta_DataSource
      )
      const userPortalFilteredRowCount = await table.VisibleRowCount()
      expect(userPortalFilteredRowCount).toBeGreaterThanOrEqual(1)

      // Verify Update and Delete Document menu items are not available for this non-ClaimsPortal document
    ...
    test('Export Document - Verify Drawer UI', async ({ browser }) => {
      ...
      exportDocumentDrawer.VerifyTitle()

      // check the Claim checkbox to display the visibility options
      ...
      await expect(exportDocumentDrawer.CheckBox_GroupVisibility_Estimator.locator).toBeAttached()
      ...
      await expect(exportDocumentDrawer.Title.locator).not.toBeAttached()
      await documentsTab.page.waitForTimeout(1000)
    ...
    test('Update Document Visiblity - Verify Drawer UI', async ({ browser }) => {
      ...
      await expect(updateDocumentVisiblityDrawer.Title.locator).not.toBeAttached()
      await documentsTab.page.waitForTimeout(1000)
    ...
    test('Documents Table - Pagination : Navigation Buttons', async ({ browser }) => {
      ...
      if (pageData.currentPage != lastPage) {
