...
test.describe(
  'NavBar',
  {
    tag: [Tags.Delegate, Tags.InspectionTech, Tags.NavBar],
  },
  () => {
    test('Verify InspectionTech LeftNavBar', async ({ browser }) => {
      // launch the Delegate Inspection Tech home page
      const { global, homePage } = await LaunchInspectionTech(browser, environment)
      ...
      // Verify Schedule page navigation from LeftNavBar
      const delegateSchedulesPage = new DelegatePortalClaimInspectionsSchedulePage(global)
    ...
    test('Update License Number Drawer - Validate', async ({ browser }) => {
      ...
      // Set licence number to empty
      await updateLicenseNumberDrawer.TextBox_LicenseNumber.locator.clear()
    ...
    test('Verify InspectionTech Login and Logout', async ({ browser }) => {
      // launch the Delegate Inspection Tech in a state that forces us to authenticate, and also to logout
