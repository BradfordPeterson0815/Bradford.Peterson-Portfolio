...
import { DefaultEnvironment, NicelyFormedClaimsPortalAuthOrigins } from '../../library/claimsPortal/claimsPortalConstants.js'
import { Launch } from '../../library/claimsPortal/claimsPortalHelper.js'
import { cookieType } from '../../library/shared/constants.js'

const authFile = claimsPortal.AUTH_STORAGE_PATH
const environment = DefaultEnvironment

setup.use({ storageState: authFile })
setup('AuthenticateClaimsAsNeeded', async ({ browser }) => {
...
async function SpikeAuthentication(forceAuthentication = false) {
  const baseUrl = claimsPortal.BASE_URL
  ...
  try {
    ...
    console.error('Error reading ClaimsPortal session storage JSON file:', error)
  }
}
