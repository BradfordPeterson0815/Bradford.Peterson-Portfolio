...
import { UserPortalTableFilterDialog } from './dialogs/userPortalTableFilterDialog.js'
import { UserPortalTableSearchDialog } from './dialogs/userPortalTableSearchDialog.js'
import { UserPortalTableSettingsDialog } from './dialogs/userPortalTableSettingsDialog.js'
...
export class UserPortalDataTable extends UserPortalBase {
  ...
  async OpenTableSettings(): Promise<UserPortalTableSettingsDialog> {
    await this.Button_OpenTableSettings.Click()
    const tableSettingsDialog = new UserPortalTableSettingsDialog(this.global)
    await tableSettingsDialog.Title.locator.waitFor({ state: 'visible', timeout: 4000 })
    return tableSettingsDialog
  }

  async OpenTableSearch(): Promise<UserPortalTableSearchDialog> {
    await this.Button_OpenTableSearch.Click()
    const tableSearchDialog = new UserPortalTableSearchDialog(this.global)
  ...
  async SetTableSearch(searchTerm = '', skipClose = false) {
    ...
    await tableSearchDialog.page.waitForTimeout(1000)
    if (!skipClose) {
  ...
  async AddTableFilter(
    ...
    isEditMode = false
  ): Promise<UserPortalTableFilterDialog> {
    ...
    await this.page
      ...
      .click()
    const tableFilterDialog = new UserPortalTableFilterDialog(this.global, isEditMode)
