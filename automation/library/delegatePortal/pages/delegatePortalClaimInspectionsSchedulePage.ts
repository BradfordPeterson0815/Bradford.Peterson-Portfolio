...
export class DelegatePortalClaimInspectionsSchedulePage extends DelegatePortalBasePage {
  ...
  constructor(global: DelegatePortalGlobal) {
    ...
    this.Button_Previous = new Element(
      global.page,
      this.page.getByRole('button', { name: JobInspectionsSchedulePageStrings.Button_Previous })
    )
    this.Button_Next = new Element(
      global.page,
      this.page.getByRole('button', { name: JobInspectionsSchedulePageStrings.Button_Next })
    )
    this.Label_CurrentMonth = new Element(
      global.page,
      this.page
        .locator(
          `#root div.chakra-card__body p[title="${JobInspectionsSchedulePageStrings.Label_CurrentMonth}"]`
        )
        .nth(0)
    )
  ...
  async GetEventsForVisibleDate(day: number, onlyScheduledEvents = false) {
    ...
    for (let i = 0; i < eventCount; i++) {
      ...
      const seeNotesLocator = specificEventForTargetDay
        .getByRole('button', {
          name: JobInspectionsSchedulePageStrings.Button_SeeNotes,
        })
        .first()
      const actionsLocator = specificEventForTargetDay
        .getByRole('button', {
          name: JobInspectionsSchedulePageStrings.Button_Actions,
        })
        .first()
  ...
  async GetEventsForVisibleMonth(onlyScheduledEvents = false) {
    const allEventsForVisibleMonth = this.page
      .locator('div[id$="_content"] div[title*="Events for "] ul')
      .first()
      .locator('li:nth-child(1)')
