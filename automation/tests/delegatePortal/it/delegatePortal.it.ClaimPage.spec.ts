...
test.describe(
  'Claim Page',
  {
    tag: [Tags.Delegate, Tags.InspectionTech, Tags.Claim],
  },
  () => {
    test('Verify Navigation and UI Elements', async ({ browser }) => {
      test.slow()
      // launch the Delegate Inspection Tech home page
      const { homePage } = await LaunchInspectionTech(browser, environment)
      ...
      // Verify that button <-Claims is displayed (top left)
      expect(await claimPage.Button_Claims.IsVisible()).toBe(true)
      ...
      const claimNumber = await claimPage.Label_ClaimNumber.GetText()
      expect(claimNumber).toBe(testClaim.basicInfo.claimNumber)
      ...
      // Click the <-Claims button and verify we are back on the Claims page
      await claimPage.Button_Claims.Click()
      await claimPage.page.waitForTimeout(1000)
      await homePage.VerifyTitle()
    })

    test('Verify Mobile Navigation and UI Elements', async ({ browser }) => {
      // launch the Delegate Inspection Tech in mobile mode
      const { homePage } = await LaunchInspectionTechMobile(browser, environment)
      ...
      // Verify that button <- All Claims is displayed (top left)
      expect(await claimPage.Link_AllClaims.IsVisible()).toBe(true)
      ...
      // Select ScheduleTab section link- verify the ScheduleTab tab appears
      ...
      // Click the <-Claims button and verify we are back on the Claims page
      await claimPage.Link_AllClaims.Click()
      await claimPage.page.waitForTimeout(1000)
      await homePage.VerifyTitle()
    })
  }
