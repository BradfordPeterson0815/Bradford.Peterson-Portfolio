...
test.describe(
  'Job Communication Page',
  {
    tag: [Tags.UserPortal, Tags.Job, Tags.Communication],
  },
  () => {
    test('Verify Navigation and UI Elements', async ({ browser }) => {
      ...
      // Verify job number
      await communicationPage.VerifyJobNumber()
      ...
      await communicationPage.VerifyContactInfo(0, job.testData.claimContact)

      // Verify job contact info
      await communicationPage.VerifyContactInfo(1, job.testData.jobContact)
      ...
      await expect(
        communicationPage.Button_RequestCallback_YourProjectManager.locator
      ).toBeVisible()
    })
    ...
    test('Request Callback - Verify Drawer UI', async ({ browser }) => {
      ...
      // Open the Request Callback Drawer from Your Project Manager
      let requestCallbackDrawer =
        await communicationPage.OpenRequestCallbackDrawerForYourProjectManager()

      //Verify drawer heading is "Request Callback from Your Project Manager"
