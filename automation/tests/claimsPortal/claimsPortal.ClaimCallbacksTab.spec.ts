...
import { ClaimsPortalClaimCallbackRequestsTab } from '../../library/claimsPortal/tabs/claimsPortalClaimCallbackRequestsTab.js'
...
test.describe(
  ...
  {
    tag: [Tags.ClaimsPortal, Tags.Claim, Tags.Callbacks],
  },
  () => {
    test('Smoke Test', { tag: [Tags.Smoke, Tags.P1] }, async ({ browser }) => {
      ...
      const callbacksTab = (await claimPage.SelectClaimTab(
        ClaimTabTypes.CallbackRequests
      )) as ClaimsPortalClaimCallbackRequestsTab
    ...
    test('Callbacks Table - Table Filter: Verify UI', async ({ browser }) => {
      ...
      //If the table is empty, we cannot perform this test
      if (await table.IsEmpty()) {
        AbortTest(AbortErrors.EmptyClaimActiveCallbackRequestsTableMessage)
        return
      }
    ...
    test('Callbacks Table - Table Filter: Add Filter', async ({ browser }) => {
      ...
      const notesFilterTerm = testClaim.testData.callbackSearch
    ...
    test('Update Callback Status - Verify Drawer UI', async ({ browser }) => {
      ...
      //If the table is empty, we cannot perform this test
      if (await table.IsEmpty()) {
        AbortTest(AbortErrors.EmptyClaimCompletedCallbackRequestsTableMessage)
        return
      }
