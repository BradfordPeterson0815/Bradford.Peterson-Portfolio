...
import { ClaimsPortalEstimateDetailsPage } from '../../library/claimsPortal/pages/claimsPortalEstimateDetailsPage.js'
...
import { ClaimsPortalClaimEstimatesTab } from '../../library/claimsPortal/tabs/claimsPortalClaimEstimatesTab.js'
...
test.describe(
  ...
  {
    tag: [Tags.ClaimsPortal, Tags.Claim, Tags.Estimates, Tags.InfoDetails],
  },
  () => {
    test('Smoke Test', { tag: [Tags.Smoke, Tags.P2] }, async ({ browser }) => {
      ...
      )) as ClaimsPortalClaimEstimatesTab
      expect(await claimPage.IsTabActive(ClaimTabTypes.Estimates)).toBe(true)
      expect(claimPage.page.url()).toBe(estimatesTab.URL)
      const estimatesTable = estimatesTab.DataTable_Estimates

      // Verify Estimates Table exists and check if it is empty
      expect(await estimatesTable.IsVisible()).toBe(true)
      const isEmpty = await estimatesTable.IsEmpty()

      if (!isEmpty) {
        // open the estimate details page
        const rowIndex = await estimatesTable.FetchRowIndexFromRowPosition(1)
        await estimatesTable.ClickLinkInDataCell_ProvideName(rowIndex, 'actionMenu')
        const estimateDetailsPage = new ClaimsPortalEstimateDetailsPage(global, testClaim, claimPage.baseURL)
        const estimateDetailsDocumentsTable = estimateDetailsPage.DataTable_ClaimDocuments
        ...
        await estimateDetailsPage.VerifyDetailsSection(true)
        await estimateDetailsPage.VerifyNotesSection(true)
        await estimateDetailsPage.VerifyReviewsSection()

        // Verify global filter works
        const initialRowCount = await estimateDetailsDocumentsTable.VisibleRowCount()

        // Verify setting search input causes the table results to filter across all text fields
        await estimateDetailsDocumentsTable.SetTableSearch('NoMatchExpected')

        const filteredRowCount = await estimateDetailsDocumentsTable.VisibleRowCount()
        expect(filteredRowCount).toBe(0)

        //  and clicking X button on it removes it and clears the search
        await estimateDetailsDocumentsTable.CancelPinnedTableSearch()

        // Verify table is NOT filtered anymore
        expect(await estimateDetailsDocumentsTable.IsGlobalSearchActive()).toBe(false)
        const filterCanceled = await estimateDetailsDocumentsTable.VisibleRowCount()
