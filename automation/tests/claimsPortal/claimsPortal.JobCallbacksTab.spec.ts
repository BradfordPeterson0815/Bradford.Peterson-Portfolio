...
import { ClaimsPortalJobCallbackRequestsTab } from '../../library/claimsPortal/tabs/claimsPortalJobCallbackRequestsTab.js'
...
test.describe(
  'Job Page: Callbacks Tab',
  {
    tag: [Tags.ClaimsPortal, Tags.Job, Tags.Callbacks],
  },
  () => {
    test('Smoke Test', { tag: [Tags.Smoke, Tags.P1] }, async ({ browser }) => {
      ...
      //Verify that we land on the Job page for that job (Info tab by default), then navigate to the Callbacks tab
      const callbacksTab = (await jobPage.SelectJobTab(
        JobTabTypes.CallbackRequests
      )) as ClaimsPortalJobCallbackRequestsTab
      expect(await jobPage.IsTabActive(JobTabTypes.CallbackRequests)).toBe(true)
      expect(jobPage.page.url()).toBe(callbacksTab.URL)
      ...
      await callbacksTab.VerifyTableSettingColumns()
      ...
      if (await table.IsEmpty()) {
        ...
        const updateCallbackStatusDrawer =
          await callbacksTab.OpenUpdateCallbackStatusDrawer(firstRowIndex)
    ...
    test('Callbacks Table - Settings: Verify UI', async ({ browser }) => {
      ...
      //Verify that we land on the Job page for that job, then navigate to the Callback Requests tab
      const callbacksTab = (await jobPage.SelectJobTab(
        JobTabTypes.CallbackRequests
      )) as ClaimsPortalJobCallbackRequestsTab
      const table = callbacksTab.DataTable_Callbacks
    ...
    test('Callbacks Table - Global Search: Verify search', async ({ browser }) => {
      ...
      // Verify setting search input causes the table results to filter across all text fields
      const descriptionSearchTerm = testJob.testData.callbackSearch
    ...
    test('Callbacks Table - Table Filter: Verify UI', async ({ browser }) => {
      ...
      //If the table is empty, we cannot perform this test
      if (await table.IsEmpty()) {
        AbortTest(AbortErrors.EmptyJobActiveCallbackRequestsTableMessage)
        return
      }
    ...
    test('Callbacks Table - Table Filter: Add Filter', async ({ browser }) => {
      ...
      const notesFilterTerm = testJob.testData.callbackSearch
    ...
    test('Update Callback Status - Verify Drawer UI', async ({ browser }) => {
      ...
      if (!(await table.IsEmpty())) {
        rowIndex = await table.FetchRowIndexOfDataByColumnName(
          testJob.testData.callbackSearch,
          DataTable_Columns_Type.Callbacks_Notes
        )
      }
      ...
      let updateCallbackStatusDrawer = await callbacksTab.OpenUpdateCallbackStatusDrawer(rowIndex)
      ...
      updateCallbackStatusDrawer = await callbacksTab.OpenUpdateCallbackStatusDrawer(rowIndex)
    ...
    test('Update Callback Status - Validate Drawer', async ({ browser }) => {
      ...
      const updateCallbackStatusDrawer = await callbacksTab.OpenUpdateCallbackStatusDrawer(rowIndex)
    ...
    test('Verify Change Callback Status', async ({ browser }) => {
      ...
      await callbacksTab.UpdateCallbackStatus(rowIndex, CallbackStatusSelectionOptions.Requested)
      ...
      await callbacksTab.UpdateCallbackStatus(rowIndex, CallbackStatusSelectionOptions.Pending)
      await callbacksTab.Reload()
      currentState = await table.FetchRowTextDataByColumnName(
        rowIndex,
        DataTable_Columns_Type.Callbacks_Status
      ...
      await callbacksTab.UpdateCallbackStatus(rowIndex, CallbackStatusSelectionOptions.Attempted)
      await callbacksTab.Reload()
      currentState = await table.FetchRowTextDataByColumnName(
        rowIndex,
        DataTable_Columns_Type.Callbacks_Status
