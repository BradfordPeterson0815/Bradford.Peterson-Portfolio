...
import { ClaimsPortalJobPortalAccessTab } from '../../library/claimsPortal/tabs/claimsPortalJobPortalAccessTab.js'
...
test.describe(
  'Job Page: Portal Access Tab',
  {
    tag: [Tags.ClaimsPortal, Tags.Job, Tags.PortalAccess],
  },
  () => {
    test('Smoke Test', { tag: [Tags.Smoke, Tags.P1] }, async ({ browser }) => {
      ...
      const portalAccessTab = (await jobPage.SelectJobTab(
        JobTabTypes.PortalAccess
      )) as ClaimsPortalJobPortalAccessTab
      expect(await jobPage.IsTabActive(JobTabTypes.PortalAccess)).toBe(true)
      expect(jobPage.page.url()).toBe(portalAccessTab.URL)
    ...
    test('Portal Access Table - Global Search: Verify search', async ({ browser }) => {
      ...
      const nameSearchTerm = testJob.testData.jobPortalAccessContact
    ...
    test('Portal Access Table - Table Filter: Verify UI', async ({ browser }) => {
      ...
      //If the table is empty, we cannot perform this test
      if (await table.IsEmpty()) {
        AbortTest(AbortErrors.EmptyJobPortalAccessTableMessage)
        return
      }
    ...
    test('Portal Access Table - Table Filter: Add Filter', async ({ browser }) => {
      ...
      const contactFilterTerm = testJob.testData.jobPortalAccessContact
      ...
      const { tableFilterDialog } = await table.SetTableFilter_Selection(
        PortalAccessStatusSelectionOptions.Inactive,
    ...
    test('Add Person To Portal - Verify Drawer UI', async ({ browser }) => {
      ...
      // Verify drawer heading is "Add Person To Portal"
      addPersonToPortalDrawer.VerifyTitle()
      expect(addPersonToPortalDrawer.claimContact).toBeAttached()
      expect(addPersonToPortalDrawer.CheckBox_AddNote.locator).toBeAttached()
      expect(await addPersonToPortalDrawer.CheckBox_AddNote.locator.locator('..').isChecked()).toBe(
        false
      )
