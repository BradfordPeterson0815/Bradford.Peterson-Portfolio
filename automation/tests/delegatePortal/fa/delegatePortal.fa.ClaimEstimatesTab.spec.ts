...
test.describe(
  ...
  {
    tag: [Tags.Delegate, Tags.FieldAgent, Tags.Claim, Tags.Estimates],
  },
  () => {
    ...
    test('Estimates Table - Pagination: Show List', async ({ browser }) => {
      ...
      //Verify the Estimates table displayed rows updates to either all rows if < page size or page size  rows if > 50
