...
test.describe(
  ...
  {
    tag: [Tags.ClaimsPortal, Tags.Claim, Tags.Estimates],
  },
  () => {
    test('Smoke Test', { tag: [Tags.Smoke, Tags.P1] }, async ({ browser }) => {
      ...
      // Verify Title
      await estimatesTab.Title.VerifyExpectedText()

      // Verify Estimates Table layout...
      // Verify Estimates Column Settings / Filters / Expand button
      ...
      await estimatesTab.VerifyTableSettingColumns()
    ...
    test('Estimates Table - Pagination:LVerify UI', async ({ browser }) => {
      // launch the Claims Portal
      const { global } = await Launch(browser, environment)
    ...
    test('Estimates Table - View Estimate: Verify Navigation', async ({ browser }) => {
      // launch the Claims Portal
      const { global } = await Launch(browser, environment)
