...
import { ClaimsPortalJobUploadTab } from '../../library/claimsPortal/tabs/claimsPortalJobUploadTab.js'
...
test.describe(
  'Job Page: Upload Tab',
  {
    tag: [Tags.ClaimsPortal, Tags.Job, Tags.Upload],
  },
  () => {
    test('Smoke Test', { tag: [Tags.Smoke, Tags.P2] }, async ({ browser }) => {
      ...
      // Go to a random job page and then to the associated upload page
      const jobsPage = new ClaimsPortalJobsPage(global)
      await jobsPage.NavigateToPage()
      const { jobPage, testJob } = await jobsPage.OpenRandomJob()
      const uploadsTab = new ClaimsPortalJobUploadTab(global, testJob, jobPage.baseURL)
