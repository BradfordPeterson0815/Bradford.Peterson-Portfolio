...
export class ClaimsPortalSelectionDataTable extends ClaimsPortalDataTable {
  ...
  async CancelRowSelection() {
    const buttonLocator = this.selectionBadgeLocator
      .locator('..')
  ...
  async SelectRowByIndex(rowIndex: string, checked: boolean, selectColumnName = 'select') {
    if (rowIndex === 'Loading') {
      throw Error('We are getting a wierd index')
    }
