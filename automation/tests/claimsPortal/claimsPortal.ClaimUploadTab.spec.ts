...
import { ClaimsPortalClaimUploadTab } from '../../library/claimsPortal/tabs/claimsPortalClaimUploadTab.js'
...
test.describe(
  ...
  {
    tag: [Tags.ClaimsPortal, Tags.Claim, Tags.Upload],
  },
  () => {
    test('Smoke Test', { tag: [Tags.Smoke, Tags.P2] }, async ({ browser }) => {
      ...
      // go to the associated uploads page
      const uploadsTab = new ClaimsPortalClaimUploadTab(global, testClaim, claimPage.baseURL)
