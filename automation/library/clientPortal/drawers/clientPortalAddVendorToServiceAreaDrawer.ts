...
export class ClientPortalAddVendorToServiceAreaDrawer extends ClientPortalBase {
  ...
  private targetVendor: Vendor | null
  private targetServiceArea: ServiceArea

  constructor(global: ClientPortalGlobal, serviceArea: ServiceArea) {
    super(global)
    this.parent = this.page.locator('div[role="dialog"]')
    const titleText = `Add Vendor to "${serviceArea.name}"`
    ...
    this.Button_CreateVendor = new Element(
      global.page,
      this.page.locator('#drawer_serviceareavendorcreation_vendorselection_button_createvendor'),
      DrawerStrings.AddVendorToServiceArea_Button_CreateVendor
    )
  ...
  async Validate() {
    // Validate Select Vendor is in an invalid state and that the error is..
    let selectionFieldValidated = false
    const selectionFieldValidator = this.page.locator(
      '#drawer_serviceareavendorcreation_vendorselection_list_vendor-feedback'
    )
  ...
  async SelectVendor(vendor: Vendor) {
    await this.page.waitForTimeout(1000)
    const selectionLocator = this.parent.locator(
      '#drawer_serviceareavendorcreation_vendorselection_list_vendor'
    )
    await selectionLocator.focus()
    await selectionLocator.clear()
    await this.page.keyboard.type(vendor.name, { delay: 20 })
