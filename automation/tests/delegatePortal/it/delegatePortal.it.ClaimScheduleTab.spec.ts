...
test.describe(
  ...
  {
    tag: [Tags.Delegate, Tags.InspectionTech, Tags.Claim, Tags.Schedule],
  },
  () => {
    ...
    test('Verify Calendar Data', async ({ browser }) => {
      ...
      for (let monthIndex = 0; monthIndex < eventNumber; monthIndex++) {
        ...
        for (let dayIndex = 0; dayIndex < dayEventsNumber; dayIndex++) {
          ...
          expect(eventDelegate).toBe(testClaim.basicInfo.inspectionTech)
        }
      }
    })
    ...
    test('Verify Calendar Entry Tooltip(s)', async ({ browser }) => {
      ...
      for (let eventIndex = 0; eventIndex < eventNumber; eventIndex++) {
        ...
        for (let dayIndex = 0; dayIndex < dayEventsNumber; dayIndex++) {
          ...
          expect(delegate).toBe(ClaimScheduleTabStrings.Tooltip_InspectionTech)
        }
      }
    })
    ...
    test('Calendar Entry Actions - Mark Completed: ValidateComplete Inspection Drawer', async ({
      browser,
    }) => {
      // launch the Delegate Inspection Tech home page
      const { global } = await LaunchInspectionTech(browser, environment)
