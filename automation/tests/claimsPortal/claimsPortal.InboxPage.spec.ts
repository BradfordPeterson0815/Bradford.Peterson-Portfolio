...
test.describe(
  'Inbox Page',
  {
    tag: [Tags.ClaimsPortal, Tags.Inbox],
  },
  () => {
    ...
    test('Smoke Test', { tag: [Tags.Smoke, Tags.P1] }, async ({ browser }) => {
      ...
      // Verify Admin->Inbox Page navigation from ClaimsPortalLeftNavBar
      ...
      if (await inboxPage.IsEmpty()) {
        // Verify the # Unread label (to the right of the title)
        const unreadCount = await inboxPage.UnreadMessageCount()
    ...
    test('Verify Navigation and UI Elements', async ({ browser }) => {
      ...
      if ((await inboxPage.MessageCount()) > 0) {
        ...
        const messagingCategories: string[] = [
          'Messaging - Documents',
          'Messaging - ClaimsPortal',
          'Messaging - Assignments',
          'Messaging - Job Received',
        ]
        ...
        const messagingTopics: string[] = [
          'New claim created',
          'New job created',
          'A Document was uploaded for',
          'Assignment',
        ]
        ...
        const messagingDetails: string[] = [
          ...
          'File Name ',
          `You've been set as the Primary Coordinator for `,
        ]
    ...
    test('Mark Message as Read', async ({ browser }) => {
      ...
      await inboxPage.Button_MarkUnread.Click()
      await inboxPage.WaitUntilDataLoadingIsCompleted(inboxPage.Button_MarkUnread.locator)
      ...
      await inboxPage.WaitUntilDataLoadingIsCompleted(inboxPage.Button_MarkRead.locator)
    ...
    test('Mark All Read', async ({ browser }) => {
      ...
      await inboxPage.Button_MarkAllRead.Click()
      await inboxPage.page.waitForTimeout(5000)
    ...
    test('View Claim', async ({ browser }) => {
      ...
      for (let index = 0; index < maxMessageCount; index++) {
        ...
        if (category == 'Messaging - ClaimsPortal') {
          await messageAbstract.MakeActive()
    ...
    test('View Job', async ({ browser }) => {
      ...
      for (let index = 0; index < maxMessageCount; index++) {
        const messageAbstract = await inboxPage.MessageAbstract(index)
        const category = await messageAbstract.Label_Category.GetText()
        if (category == 'Messaging - Job Received') {
          await messageAbstract.MakeActive()
          expect(await inboxPage.Link_MessageDetails_ViewJob.IsVisible()).toBe(true)

          // Follow View Job link
          const targetURL = await inboxPage.Link_MessageDetails_ViewJob.locator.getAttribute('href')
          await inboxPage.Link_MessageDetails_ViewJob.Click()
          await inboxPage.page.waitForTimeout(3000)
