...
import { FetchCannedJob, FetchDateSuffix } from '../../../library/delegatePortal/delegatePortalHelper.js'
...
test.describe(
  'PhotoReport Tab',
  {
    tag: [Tags.Delegate, Tags.FieldTech, Tags.PhotoReport],
  },
  () => {
    test('Verify Navigation and UI from Job', async ({ browser }) => {
      ...
      const photoReportTab = new DelegatePortalPhotoReportTab(global, `jobs/${testJob.jobDetails.jobId}`)
    ...
    test.describe('serial tests', () => {
      ...
      test('Verify Card Drag Drop in same Group', async ({ browser }) => {
        ...
        await photoReportTab.DragCardSelection(
          [initialSecondCard, initialFirstCard],
          initialThirdCard,
          1
        )
    ...
    test('Validate Add Removed Photos', async ({ browser }) => {
      ...
      const { title: restoreFirstTitleGroupTwo, description: restoreFirstDescriptionGroupTwo } =
        await restoreFirstCardGroupTwo.FetchCardInfo()

      // first card from group two should be the original card in group two
      expect(restoreFirstTitleGroupTwo).toBe(initialFirstTitleGroupTwo)
      expect(restoreFirstDescriptionGroupTwo).toBe(initialFirstDescriptionGroupTwo)
    })
