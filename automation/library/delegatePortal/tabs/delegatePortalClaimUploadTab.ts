...
import { MaxUploadFiles, UploadTabStrings, ValidationStrings } from '../delegatePortalConstants.js'
import { DelegatePortalFileCard } from '../delegatePortalFileCard.js'
export class DelegatePortalClaimUploadTab extends DelegatePortalBasePage {
  ...
  readonly URL: string
  readonly InspectionsUploadURL: string
  ...
  constructor(global: DelegatePortalGlobal, claim: DelegatePortalClaim, claimPageURL: string) {
    super(global)
    this.claim = claim
    this.URL = `${claimPageURL}/upload`
    this.InspectionsUploadURL = `${claimPageURL}/inspections/new-from-upload`
  ...
  FetchFileCard(index: number) {
    const fileCard = new DelegatePortalFileCard(this.global, this.fileCards, index)
    return fileCard
  }
  ...
  async UploadValidButUnsupportedInspectionFile() {
