...
import { DelegatePortalCreateNoteDrawer } from '../../../library/delegatePortal/drawers/delegatePortalCreateNoteDrawer.js'
...
test.describe(
  'Callbacks Page',
  {
    tag: [Tags.Delegate, Tags.InspectionTech, Tags.Callbacks],
  },
  () => {
    test(' Verify Navigation and UI Elements', async ({ browser }) => {
      ...
      // Verify Callbacks page navigation from DelegateLeftNavBar
    ...
    test.describe('serial tests', () => {
      ...
      test('Callbacks Table - Sort Columns', async ({ browser }) => {
        ...
        // Verify Admin->Callbacks page navigation from DelegateLeftNavBar
        const callbacksPage = new DelegatePortalCallbacksPage(global)
        await callbacksPage.NavigateToPage()
        const table = callbacksPage.DataTable_Callbacks
    ...
    test('Callbacks Table - Verify Action Menu: Change callback status', async ({ browser }) => {
      ...
      const updateCallbackStatusDrawer =
        await callbacksPage.OpenUpdateCallbackStatusDrawer(rowIndex)
    ...
    test(' Update Callback Status - Verify Drawer UI', async ({ browser }) => {
      ...
      const rowIndex = '0'
      let updateCallbackStatusDrawer = await callbacksPage.OpenUpdateCallbackStatusDrawer(rowIndex)
      ...
      updateCallbackStatusDrawer = await callbacksPage.OpenUpdateCallbackStatusDrawer(rowIndex)
    ...
    test(' Update Callback Status - Validate Drawer', async ({ browser }) => {
      ...
      // select a status
      await updateCallbackStatusDrawer.SetStatusSelection(CallbackStatusSelectionOptions.Requested)

      // Click the Submit button to move to Notes page in drawer
      await updateCallbackStatusDrawer.Button_Submit.Click()
      await callbacksPage.page.waitForTimeout(1000)

      const createNoteDrawer = new DelegatePortalCreateNoteDrawer(global)
    ...
    test(' Verify Change Callback Status', async ({ browser }) => {
      ...
      if (entityInfo.endsWith('Claim')) {
        ...
        entityInfo = entityInfo.split('Job')[0]
      }
      ...
      await callbacksPage.UpdateCallbackStatus('0', firstStatusChange)
      ...
      await callbacksPage.UpdateCallbackStatus('0', secondStatusChange)
      ...
      // Change the status and check to make sure it updated again
      await callbacksPage.UpdateCallbackStatus('0', thirdStatusChange)
      currentStatus = await table.FetchRowTextDataByColumnName(
        '0',
        DataTable_Columns_Type.Callbacks_Status
      )
