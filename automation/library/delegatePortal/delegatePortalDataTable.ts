...
import { DateFilterFormatting, LookupDataColumn } from './delegatePortalHelper.js'
import { DelegatePortalTableFilterDialog } from './dialogs/delegatePortalTableFilterDialog.js'
import { DelegatePortalTableSearchDialog } from './dialogs/delegatePortalTableSearchDialog.js'
import { DelegatePortalTableSettingsDialog } from './dialogs/delegatePortalTableSettingsDialog.js'
import { DelegatePortalBase } from './pages/delegatePortalBase.js'

export class DelegatePortalDataTable extends DelegatePortalBase {
  ...
  async IsColumnVisible(columnType: DataTable_Columns_Type) {
    ...
    await data.locator.count()
    await data.locator.isVisible()
  ...
  async OpenTableSettings(): Promise<DelegatePortalTableSettingsDialog> {
    await this.Button_OpenTableSettings.Click()
    const tableSettingsDialog = new DelegatePortalTableSettingsDialog(this.global)
    await tableSettingsDialog.Title.locator.waitFor({ state: 'visible', timeout: 4000 })
    return tableSettingsDialog
  }

  async OpenTableSearch(): Promise<DelegatePortalTableSearchDialog> {
    await this.Button_OpenTableSearch.Click()
    const tableSearchDialog = new DelegatePortalTableSearchDialog(this.global)
  ...
  async AddTableFilter(
    ...
    isEditMode = false
  ): Promise<DelegatePortalTableFilterDialog> {
    ...
    await this.page
      ...
      .click()
    const tableFilterDialog = new DelegatePortalTableFilterDialog(this.global, isEditMode)
  ...
  async SetTableFilter_Date(
    ...
    switch (dateFilterType) {
      ...
      case DateFilterTypes.DateLesserEqualThan:
        ...
        break
      case DateFilterTypes.DateTBD:
        comparison = 'equals'
        break
    }
    const pinnedFilter =
      dateFilterType == DateFilterTypes.DateTBD
        ? `${filter} ${comparison} "TBD"`
        : `${filter} ${comparison} "${DateFilterFormatting(date)}"`
    await this.page.waitForTimeout(1000)
    return { pinnedFilter, tableFilterDialog }
  }
