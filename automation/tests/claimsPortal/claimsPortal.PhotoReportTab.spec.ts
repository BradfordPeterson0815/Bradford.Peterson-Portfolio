...
import { ClaimsPortalJobMediaTab } from '../../library/claimsPortal/tabs/claimsPortalJobMediaTab.js'
import { ClaimsPortalPhotoReportTab } from '../../library/claimsPortal/tabs/claimsPortalPhotoReportTab.js'
...
test.describe(
  'PhotoReport Tab',
  {
    tag: [Tags.ClaimsPortal, Tags.PhotoReport],
  },
  () => {
    test('Smoke Test', { tag: [Tags.Smoke, Tags.P2] }, async ({ browser }) => {
      ...
      const { testClaim } = await claimsPage.OpenRandomClaim()

      // Go straight to the photo report tab
      const photoReportTab = new ClaimsPortalPhotoReportTab(
        global,
        `claims/${testClaim.basicInfo.claimNumber}`
      )
      await photoReportTab.NavigateDirectly()

      if (await photoReportTab.IsTabEmpty()) {
        await photoReportTab.Label_Empty_Title.VerifyExpectedText()
      } else {
    ...
    test('Verify Navigation and UI from Claim', async ({ browser }) => {
      ...
      photoReportTab = new ClaimsPortalPhotoReportTab(global, `claims/${testClaim.basicInfo.claimNumber}`)
      await photoReportTab.page.waitForTimeout(1000)
    ...
    test('Verify Navigation and UI from Job', async ({ browser }) => {
      // launch the Claims Portal
      const { global } = await Launch(browser, environment)
      ...
      //Verify that we land on the Job page for that job (Info tab by default), then navigate to the Media tab
      const mediaTab = (await jobPage.SelectJobTab(JobTabTypes.Media)) as ClaimsPortalJobMediaTab
      expect(await jobPage.IsTabActive(JobTabTypes.Media)).toBe(true)
      expect(jobPage.page.url()).toBe(mediaTab.URL)

      // Navigate from the Media tab to the Photo Report tab
      await mediaTab.Link_CreatePhotoReport.Click()
      const photoReportTab = new ClaimsPortalPhotoReportTab(global, `jobs/${testJob.jobDetails.jobId}`)
      await photoReportTab.page.waitForTimeout(1000)
      expect(mediaTab.page.url()).toBe(photoReportTab.URL)
      ...
      expect(await photoReportTab.Button_CollapseGroups.IsVisible()).toBe(true)
      await photoReportTab.Wait(4000)
      expect(await photoReportTab.Link_DownloadLastPhotoReport.IsVisible()).toBe(false)
      expect(await photoReportTab.Button_SubmitPhotoReport.IsVisible()).toBe(true)
    })
    ...
    test('Verify Sort Order Dialog UI', async ({ browser }) => {
      ...
      await sortOrderDialog.Title.locator.waitFor({ state: 'hidden' })
      await expect(sortOrderDialog.Title.locator).not.toBeVisible()

      // Verify Sort Order dialog - closes with ESC key
    ...
    test('Verify Download Last Photo Report', async ({ browser }) => {
      // launch the Claims Portal
      const { global } = await Launch(browser, environment)
      ...
      const dowloadPageTitleToContain = `${testClaim.basicInfo.claimNumber}/PhotoReport_2026-`
      await photoReportTab.DownloadLastPhotoReportVerifyAndClose(dowloadPageTitleToContain)
    })
  }
)
