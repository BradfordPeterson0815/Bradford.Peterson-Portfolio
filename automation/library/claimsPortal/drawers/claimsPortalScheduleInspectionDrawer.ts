...
export class ClaimsPortalScheduleInspectionDrawer extends ClaimsPortalBase {
  ...
  async Validate() {
    ...
    // Validate Inspector Role Field is in an invalid state and that the error is..
    const inspectionRoleLocator = this.ListBox_InspectorRole.locator
    if ((await inspectionRoleLocator.getAttribute('aria-invalid')) == 'true') {
      const referenceId = await inspectionRoleLocator.getAttribute('aria-describedby')
      // "Invalid enum value. Expected 'fieldAgent' | 'inspectionTech', received ''"
      inspectorRoleIsValidated =
        (await this.page.locator(`div[id='${referenceId}']`).textContent()) ==
        ValidationStrings.InvalidEnumInspectorRole
    }
    ...
    return inspectionDateIsValidated && inspectorRoleIsValidated && scheduledByFieldIsValidated
  }

  async ValidateWithRole() {
    let inspectionDateIsValidated = false
    let scheduledByFieldIsValidated = false
