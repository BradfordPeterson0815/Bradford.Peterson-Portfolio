...
test.describe(
  'NavBar',
  {
    tag: [Tags.Delegate, Tags.Subcontractor, Tags.NavBar],
  },
  () => {
    test('Verify Subcontractor LeftNavBar', async ({ browser }) => {
      // launch the Delegate Subcontractor home page
      const { global, homePage } = await LaunchSubcontractor(browser, environment)
      ...
      // TBD - Add Mobile App Links Check
    ...
    test('Verify Subcontractor Login and Logout', async ({ browser }) => {
      // launch the Delegate Subcontractor in a state that forces us to authenticate, and also to logout
