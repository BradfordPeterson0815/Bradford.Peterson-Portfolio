...
test.describe(
  'NavBar',
  {
    tag: [Tags.Delegate, Tags.FieldAgent, Tags.NavBar],
  },
  () => {
    test('Verify FieldAgent LeftNavBar', async ({ browser }) => {
      // launch the Delegate Field Agent home page
      const { global, homePage } = await LaunchFieldAgent(browser, environment)
    ...
    test('Verify FieldAgent Login and Logout', async ({ browser }) => {
      // launch the Delegate Field Agent in a state that forces us to authenticate, and also to logout
