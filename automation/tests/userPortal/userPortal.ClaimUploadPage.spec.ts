...
test.describe(
  'Claim Upload Page',
  {
    tag: [Tags.UserPortal, Tags.Claim, Tags.Upload],
  },
  () => {
    test('Smoke Test', { tag: [Tags.Smoke, Tags.P2] }, async ({ browser }) => {
      // launch UserPortal and navigate to Upload page on a claim
      const { global, activeClaimsAndJobsPage } = await Launch(browser, environment)
      const { claimPage, testClaim } = await activeClaimsAndJobsPage.OpenRandomClaim()
      const uploadPage = new UserPortalClaimUploadPage(global, testClaim, claimPage.baseURL)
    ...
    test('Verify Navigation and UI', async ({ browser }) => {
      ...
      const documentsPage = new UserPortalClaimDocumentsPage(global, testClaim)
      ...
      let uploadPage = new UserPortalClaimUploadPage(global, testClaim, claimPage.baseURL)
      expect(documentsPage.page.url()).toBe(uploadPage.URL)

      // Head over to the Media page
      const mediaPage = new UserPortalClaimMediaPage(global, testClaim)
      ...
      uploadPage = new UserPortalClaimUploadPage(global, testClaim, mediaPage.baseURL)
